...
States:  Amount of water in each respective jug, where the states are represented by
 ...
 Our Initial State: [0,0] means jugs are initially empty
...
def get_goal(jugs):
    ...
    s = "Enter the required  amount of water (1 - {0}): ".format(max_amount)
    goal_amount = int(input(s))
    while goal_amount < 1 or goal_amount > max_amount:
        goal_amount = int(input("Enter  valid amount (1 - {0}): ".format(max_amount)))
...
def been_there(node, check_dict):
    ...
    print("Checking if {0} is visited before or not ".format(node))

    return check_dict.get(get_index(node), False)

def next_transitions(jugs, path, check_dict):
    """
    Returns list of all possible transitions which do not cause loops
    ...
    # 6. empty second jug to ground
    node.append(a)
    node.append(0)
    if not been_there(node, check_dict):
        next_nodes.append(node)

    # create a list of next paths to be visited
